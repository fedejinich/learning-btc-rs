...
mod test_taproot {
    ...
    use bitcoin::{Address, Amount, Network, ScriptBuf, Sequence, TapLeafHash, Transaction, TxIn, TxOut, Witness};
    ...
    #[test]
    fn test_p2tr_script_path_spend() {
        ...
        let (x_pub_key, _) = keypair.x_only_public_key();
        let script = script! {
            <x_pub_key.serialize().to_vec()>
            OP_CHECKSIGVERIFY
        };
        let tr_spend_info = TaprootBuilder::new()
            .add_leaf_with_ver(0, script.clone(), LeafVersion::TapScript)
            .unwrap()
            .finalize(&secp, x_pub_key)
            .expect("couldn't create tr_spend_info");

        // Get a keypair we control. In a real application these would come from a stored secret.
        let tr_address= Address::p2tr(&secp, x_pub_key, tr_spend_info.merkle_root().clone(),
                                      bitcoin_network);

        println!("tr_address: {:?}", tr_address.clone());
        println!("tr_merkle_root: {:?}", &tr_spend_info.merkle_root());
        ...
        let input_index = 0;
        let tap_leaf_hash = TapLeafHash::from_script(script.as_script(), TapScript);
        ...
        let sighash = sighasher
            .taproot_script_spend_signature_hash(
                input_index,
                &Prevouts::All(vec![utxo].as_slice()),
                tap_leaf_hash,
                sighash_type
            )
